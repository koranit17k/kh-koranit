---
title: about
description: Welcome to Docus theme documentation.
navigation:
  icon: i-lucide-user
seo:
  title: about
  description: Discover how to create, manage, and publish documentation effortlessly with Docus.
---

::u-page
  :::u-page-grid
    :::u-page-feature
    ---
    ---
    #description
    <img src="/images/koranit.jpeg" alt="koranit" width="180" style="border-radius:16px; margin-bottom:12px;" />
    :::

    :::u-page-feature
    ---
    icon: i-heroicons-map-pin
    ---
    #title
    [ข้อมูลพื้นฐาน]{.text-primary}

    #description
    **ชื่อ:** นาย กรณิศ แก้วมณี  
    **ชื่อเล่น:** กาย  
    **อายุ:** 21 ปี  
    **เพศ:** ชาย
    **ส่วนสูง:** 175 ซม.  
    **น้ำหนัก:** 57 กก.  
    **Hometown:** ภูเก็ต, ประเทศไทย  
    **บุคลิกภาพ:** Introvert, เป็นมิตร, ชอบเรียนรู้อยู่เสมอ
    :::

    :::u-page-feature
    ---
    icon: i-heroicons-heart
    ---
    #title
    [งานอดิเรก]{.text-primary}

    #description
    ชอบเล่นเกมแนว FPS และ Turn-based ซึ่งช่วยฝึกสมาธิ การตัดสินใจ และการวางกลยุทธ์ รวมถึงชอบดูหนังและซีรีส์จีนเพื่อผ่อนคลาย และออกกำลังกายด้วยการวิ่งและตีแบดมินตันเป็นประจำ
    :::
  :::
::

---

### สายอาชีพที่สนใจ
Web Dev, Backend, Tester, Data Engineering และ Data Analysis


---

## 🎓 **การศึกษา**

#### **ปริญญาตรี**

- **วุฒิการศึกษา:** วิทยาศาสตรบัณฑิต (ธุรกิจดิจิทัล)
- **สาขา:** ธุรกิจดิจิทัล
- **คณะ** วิทยาลัยการคอมพิวเตอร์
- **สถาบัน:** มหาวิทยาลัยสงขลานครินทร์ วิทยาเขตภูเก็ต
- **จบการศึกษาปี:** 2026
- **เกรดเฉลี่ยสะสม:** 3.66

#### **มัธยมศึกษา**

- **แผนการเรียน:** ศิลป์–คำนวณ
- **สถาบัน:** โรงเรียนภูเก็ตวิทยาลัย
- **ปีที่จบ:** 2022


---

## 🛠 **ทักษะ**

::SkillGroup{title="ทักษะทั่วไป"}
::SkillCard{title="Problem Solving" description="🔥 Never give up, think smart"}
::
::SkillCard{title="Learning" description="📚 Always curious & improving"}
::
::SkillCard{title="Documentation" description="🧼 Clear, clean, readable"}
::
::SkillCard{title="Communication" description="🤗 Friendly & collaborative"}
::
::SkillCard{title="Data Analysis" description="🤖 Assisted by AI (supercharged)"}
::
::SkillCard{title="English (Reading/Writing)" description="👍 Fair & usable"}
::
::SkillCard{title="TOEIC" description="575"}
::
::

---

## 💻 **ทักษะ Dev**

::SkillGroup{title="Programming Languages"}
::SkillCard{title="SQL" percentage="50"}
::
::SkillCard{title="HTML & CSS" percentage="50"}
::
::SkillCard{title="Shellscript" percentage="50"}
::
::

::SkillGroup{title="Databases" }
::SkillCard{title="MySQL/MariaDB" percentage="50"}
::
::

## **Program**

::SkillGroup{title="Program/Extension"}
::SkillCard{title="PowerBI" percentage="80"}
::
::SkillCard{title="Antigravity" percentage="80"}
::
::SkillCard{title="DBeaver" percentage="60"}
::
::SkillCard{title="GitHub" percentage="60"}
::
::SkillCard{title="Jasper Report" percentage="60"}
::
::SkillCard{title="Vitest" percentage="50"}
::
::SkillCard{title="nginx" percentage="50"}
::
::

::SkillGroup{title="AI Agent"}
::SkillCard{title="ChatGPT" percentage="80"}
::
::SkillCard{title="Gemini" percentage="80"}
::
::SkillCard{title="Copilot" percentage="70"}
::
::SkillCard{title="Perplexity" percentage="60"}
::
::SkillCard{title="Antigravity" percentage="80"}
::
::

---

## **ประสบการณ์**

<div style="display: flex; justify-content: space-between;">
  <strong>สหกิจศึกษา ตำแหน่ง Data Analyst</strong>
  <span>12 Jan – 8 May 2026</span>
</div>
<div>บจก. กี่หิ้นการไฟฟ้า ภูเก็ต</div>


## 📁 **Portfolio & Projects**

> ### **🌐 PowerBI Dashboard**

> Dashboard with PowerBI + API integration.

> ### **🔗 Nuxt/Content คู่มือการฝึกงาน**

> Nuxt.js framework + Markdown content.

> ### **🔗 Report Viewer**

> Jasper Report + API integration.

---

## 📬 **ข้อมูลติดต่อ**

```json
{
  "name": "Koranit Keawmanee",
  "email": "koranit.17k@gmail.com",
  "github": "https://github.com/koranit17k",
  "website": "https://kh-koranit.vercel.app/",
  "phone": "+66 93-674-9281"
}
```

---
